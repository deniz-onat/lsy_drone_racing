"""Curvature-aware speed profile for the KaFa_1500_v12_1 reference.
...
_EPS = 1e-9
# Fix 2-1: median-filter window (samples) applied to the path curvature before the friction-circle
# speed cap, so an isolated spline-overshoot spike cannot collapse the speed; sustained curvature
# (real turns, several samples wide) is preserved.
KAPPA_MEDIAN_WINDOW = 5
...
class SpeedProfile:
    ...
    def __init__(
        ...
        # Fix 2-1: remove isolated curvature spikes (time-parameterized cubic overshoot) so they
        # do not force a spurious stop that the longitudinal passes then spread into a slow region.
        kappa = median_filter(kappa, size=KAPPA_MEDIAN_WINDOW, mode="nearest")
