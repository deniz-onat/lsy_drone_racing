"""Tunnel-constrained time-optimal MPCC core for KaFa_1500_v12_1 (acados SQP-RTI).
...
Own codegen namespace ``kafa_v12_1`` (different dimensions than v10.x -- never share; see the
...
if TYPE_CHECKING:
    ...
    from lsy_drone_racing.control.KaFa_v12_1.arc_path import TunnelArcPath
...
def _build_v11_solver(s: MPCCSettings, a_max: float) -> AcadosOcpSolver:
    """Code-generate the v10.1 OCP plus the soft tunnel constraints (kafa_v12_1 namespace)."""
    ...
    return AcadosOcpSolver(ocp, json_file="c_generated_code/kafa_v12_1.json", verbose=False)
...
class MPCC:
    ...
    def __init__(self, settings: MPCCSettings, a_max: float):
        ...
        self._tun_alpha = float(getattr(settings, "tunnel_smooth_alpha", 1.0))  # fix 1-1
        self.reset()
    ...
    def reset(self) -> None:
        ...
        self._prev_tun: tuple | None = None  # fix 1-1: last smoothed tunnel params
        self._solver.reset()
    ...
    def _smooth_tunnel(
        self,
        n_lat: NDArray[np.float64],
        w_half: NDArray[np.float64],
        h_half: NDArray[np.float64],
    ) -> tuple[NDArray[np.float64], NDArray[np.float64], NDArray[np.float64]]:
        """Per-step EMA low-pass of the tunnel params (fix 1-1).

        Ramps a reveal/replan jump in the corridor in over a few steps instead of snapping (which
        jitters the QP / destabilises the drone). Reset on a cold start (set_path); carried across
        rebase so a reveal is smoothed.
        """
        a = self._tun_alpha
        if a < 1.0 and self._prev_tun is not None:
            pn, pw, ph = self._prev_tun
            if pn.shape == n_lat.shape:
                n_lat = pn + a * (n_lat - pn)
                n_lat = n_lat / (np.linalg.norm(n_lat, axis=1, keepdims=True) + 1e-9)
                w_half = pw + a * (w_half - pw)
                h_half = ph + a * (h_half - ph)
        self._prev_tun = (np.array(n_lat), np.array(w_half), np.array(h_half))
        return n_lat, w_half, h_half
    ...
    def _set_stage_params(self, thbar: NDArray[np.float64], vth_cap: float) -> NDArray[np.float64]:
        ...
        n_lat, w_half, h_half = self._smooth_tunnel(n_lat, w_half, h_half)  # fix 1-1
