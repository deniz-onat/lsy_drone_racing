"""Tuning surface for KaFa_1500_v12_1 — the single place all parameters live.
...
class MPCCSettings:
    ...
    The OCP solved by KaFa_v12_1.mpcc; also read by KaFa_v12_1.arc_path for the path/tunnel view.
    """
    ...
    v_max: float = 3.6  # S1: raised straight/inter-gate speed budget (reveal caps keep gates slow)
    tilt_ratio: float = 0.85
    a_z_min: float = 7.0
    mu: float = 1.5
    v_theta_max: float = 3.6  # S1: progress-rate budget raised 3.2 -> 3.6 (operative speed cap)
    a_theta_max: float = 8.0
    r_dv: float = 0.01
    a_lat_max: float = 9.5  # S1: friction-circle lateral accel raised 8.5 -> 9.5 (faster corners)
    v_min: float = 1.3
    w_lag: float = 1.0
    w_accel: float = 0.02
    ramp_s: float = 2.4
    ...
    # Fix 1-1: per-step low-pass on the tunnel params fed to the MPCC (1.0 = off, lower = smoother).
    tunnel_smooth_alpha: float = 0.5
    # Fix 4: gate+obstacle "precision zone" -- a gate with a real obstacle within
    # gate_obstacle_radius is flown slower (v_gate_obstacle), tracked tighter (extra contouring
    # weight w_contour_obstacle), and through a narrower tunnel (tunnel_w_obstacle).
    gate_obstacle_radius: float = 0.4
    v_gate_obstacle: float = 2.0
    w_contour_obstacle: float = 20.0
    tunnel_w_obstacle: float = 0.06


@dataclass(frozen=True)
class ControllerSettings:
    """All configurable values used by KaFa_1500_v12_1 (resolved v11_1 settings tree)."""
