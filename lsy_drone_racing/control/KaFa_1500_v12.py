"""KaFa_1500_v12 — self-contained tunnel-MPCC racing controller.
...
``KaFa_v12`` package; it imports nothing from any other ``KaFa_*`` version.

Phase machine:

    TAKEOFF  -> mini vertical climb (PID-tracked), holding XY inside the floor-touch carve-out
    NAVIGATE -> guarded-smoothed global spline (KaFa_v12.planner), flown by the tunnel MPCC
                (KaFa_v12.mpcc) over the capped tunnel path view (KaFa_v12.arc_path), with a
...
from lsy_drone_racing.control.KaFa_v12.arc_path import CappedTunnelArcPath
from lsy_drone_racing.control.KaFa_v12.attitude import _vector_to_attitude
from lsy_drone_racing.control.KaFa_v12.feedback import CascadedPid
from lsy_drone_racing.control.KaFa_v12.mpcc import MPCC
from lsy_drone_racing.control.KaFa_v12.observation import parse_observation
from lsy_drone_racing.control.KaFa_v12.planner import ReferenceManager, gate_post_obstacles
from lsy_drone_racing.control.KaFa_v12.settings import ControllerSettings
from lsy_drone_racing.control.KaFa_v12.takeoff import TakeoffPhase
...
class KaFa1500V12(Controller):
    ...
    def __init__(self, obs: dict[str, NDArray[np.floating]], info: dict, config: dict):
        ...
        if config.env.control_mode != "attitude":
            raise ValueError("KaFa_1500_v12 requires env.control_mode = 'attitude'.")
        ...
        # The one MPCC actually flown: v11's tunnel-constrained OCP (codegen namespace kafa_v12).
