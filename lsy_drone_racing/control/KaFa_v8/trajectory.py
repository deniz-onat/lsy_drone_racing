"""Global reference planning with gate-post funnels for KaFa_1500_v8.

This is v6's observation-driven global planner (a clamped cubic spline built through all
remaining gates from the drone's current state, rebuilt whenever the target gate advances
or an observed pose shifts) with v7's gate-post funnels folded in: two virtual columns per
remaining gate so the spline routes through each opening's centre instead of clipping a
frame bar. Gates are crossed along their canonical +x axis (orient_gates_to_travel=False)
to match the direction the env counts a pass.

The numeric primitives (lateral nudging, obstacle push-off, reversal turns) are reused
from kafa1500_v6.avoidance; the spline timing/repair is v8's own KaFa_v8.timing.
"""
...
def _clearance_points(
    ...
    """Return a clearance + turn-apex waypoint when the next gate sits at a different height.

    next_forward must be the same travel-oriented axis the main loop uses for the next
    gate, so the turn-apex lands on the side the drone actually enters from.
    """
    ...
    # swing. Marginal ~90 deg turns keep the forward clearance, which holds up when the gate
    # reveal flips the sign of a near-perpendicular dot product.
    if cos_to_next < -0.3:
...
def _insert_exited_clearance(
    ...
    """Re-create the post-gate arc when replanning near the gate we just exited.

    The arc extends along the drone's actual exit momentum so the clamped spline carries on
    with the current motion instead of reversing into the gate we just passed.
    """
...
def build_waypoints(
    ...
    # oriented relative to the previous gate's exit, not the running waypoint chain, so the
    # choice is stable and the clearance turn-apex agrees with the gate-entry side.
...
class ReferenceManager:
    """Build and globally replan the funnelled reference spline from observed poses."""
    ...
    def _planning_obstacles(self, frame: DroneObservation) -> NDArray[np.float64]:
        """Augment the real obstacles with gate-post funnel columns for the spline build."""
    ...
    def build(
        ...
        return ReferencePlan(
            ...
            # Snapshot the REAL obstacles (not the funnel-augmented set) so the replan
            # check compares like-for-like with the next frame's obstacle array.
