"""KaFa_1500_v12_1 — self-contained tunnel-MPCC racing controller.

A clean, standalone clone of KaFa_1500_v11_1: the v11 tunnel-constrained, de-paced time-optimal
MPCC flying v10.6's guarded-smoothed, parity-capped reference. Everything it needs lives in the
``KaFa_v12_1`` package; it imports nothing from any other ``KaFa_*`` version.

Phase machine:

    TAKEOFF  -> mini vertical climb (PID-tracked), holding XY inside the floor-touch carve-out
    NAVIGATE -> guarded-smoothed global spline (KaFa_v12_1.planner), flown by the tunnel MPCC
                (KaFa_v12_1.mpcc) over the capped tunnel path view (KaFa_v12_1.arc_path), with a
                fast launch ramp and v10.2's dynamics-aware predicted-progress anchor.

This controller is a behaviour-preserving consolidation of the v9 -> v10 -> v10.1 -> v10.3 ->
v10.4 -> v10.5 -> v11 -> v11.1 class chain: the resolved ``__init__`` builds only the objects that
survive (the v11 tunnel solver, v10.6 planner, v10.4 takeoff) and the live methods are inlined
(``_track_action`` is v11.1's). It was verified to produce bit-identical control output to
KaFa_1500_v11_1. REQUIRES the acados environment -- run under ``pixi run``.
"""
...
from lsy_drone_racing.control.KaFa_v12_1.arc_path import CappedTunnelArcPath
from lsy_drone_racing.control.KaFa_v12_1.attitude import _vector_to_attitude
from lsy_drone_racing.control.KaFa_v12_1.feedback import CascadedPid
from lsy_drone_racing.control.KaFa_v12_1.mpcc import MPCC
from lsy_drone_racing.control.KaFa_v12_1.observation import parse_observation
from lsy_drone_racing.control.KaFa_v12_1.planner import ReferenceManager, gate_post_obstacles
from lsy_drone_racing.control.KaFa_v12_1.settings import ControllerSettings
from lsy_drone_racing.control.KaFa_v12_1.takeoff import TakeoffPhase
...
class KaFa1500V121(Controller):
    """Tunnel MPCC over the guarded-smoothed reference; self-contained clone of v11.1."""

    _MODE_TAKEOFF = "TAKEOFF"
    _MODE_NAVIGATE = "NAVIGATE"

    def __init__(self, obs: dict[str, NDArray[np.floating]], info: dict, config: dict):
        """Build the planner, takeoff phase, and the v11 tunnel MPCC (the only solver built)."""
        super().__init__(obs, info, config)
        if config.env.control_mode != "attitude":
            raise ValueError("KaFa_1500_v12_1 requires env.control_mode = 'attitude'.")
        ...
        # The one MPCC actually flown: v11's tunnel-constrained OCP (codegen namespace kafa_v12_1).
