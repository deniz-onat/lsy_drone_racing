...
Why this exists: v10.1 anchors the MPCC's progress state at the GLOBAL nearest path point in a 2.0 m
forward window. On a sharp slalom the path folds back on itself near a gate, so a far leg of the fold
is spatially closer than the gate apex and the anchor snaps ~1-2 m forward across the fold in one
step -- the drone skips the gate. v10.2 instead anchors progress to the SOLVER'S OWN predicted
progress (dynamics-feasible, advances at the bounded rate vth, so it cannot teleport) and lets a
...
# above the legitimate per-step fold advance (~0.6-0.7 m) and below the fold self-approach gap (~1 m),
# so the anchor tracks the drone through the fold but can never teleport across it. See docstring.
PROJ_BAND_M = 0.6
